...
<body>
    ...
    <div>
        <h6>Jurusan : {{ $absenpenelitian[0]->jurusan}}</h6>
    </div>
    <div>
        <h6>Universitas : {{ $absenpenelitian[0]->asal_instansi }}</h6>
    </div>
    <div>
        <h6>Waktu Pelaksanaan :{{ date('d-m-Y', strtotime($absenpenelitian[0]->mulai)) }} s/d
            {{ date('d-m-Y', strtotime($absenpenelitian[0]->selesai)) }}</h6>
    </div>
    <div>
        <h6>Unit Kerja : {{ $absenpenelitian[0]->divisi }}</h6>
    </div>
    ...
    <div class="row">
        ...
        <table class="table table-bordered table-striped responsive" style="table-layout: fixed;width:100%;">
            ...
            <tbody>
                @foreach ($absenpenelitian as $rekap)
