...
<body>
    ...
    <div>
        <h6>Jurusan : {{ $absenmhss[0]->jurusan }}</h6>
    </div>
    <div>
        <h6>Universitas : {{ $absenmhss[0]->univ }}</h6>
    </div>
    <div>
        <h6>Waktu Pelaksanaan :{{ date('d-m-Y', strtotime($absenmhss[0]->mulai)) }} s/d
            {{ date('d-m-Y', strtotime($absenmhss[0]->selesai)) }}</h6>
    </div>
    <div>
        <h6>Unit Kerja : {{ $absenmhss[0]->divisi }}</h6>
    </div>
        ...
        <table class="table table-bordered table-striped responsive" style="table-layout: fixed;width:100%;">
            <thead class="text-center">
                <tr>
                    ...
                    <th style="font-size: 14px">Lokasi Kegiatan</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($absenmhss as $rekap)
    ...
    <div style="font-size: 17px;">
        <p style="margin-left:580px;">Surabaya,
            {{ date('d-F-Y', strtotime($absenmhss[0]->selesai)) }} </p>
